 ...
 * 
 * The class has been written assuming that the View or the Model are the only classes that
...
public class ControllerAddRemoveRoles {
	...
	protected static void repaintTheWindow() {
		...
		ViewAddRemoveRoles.theRootPane.getChildren().addAll(
				...
				ViewAddRemoveRoles.button_Return, ViewAddRemoveRoles.button_Logout, 
	...
	protected static void setupSelectedUser() {
		...
		if (roleCount == 0) currentRoles.append("(None)");
	 ...
	 * - Users have at least one role (if policy requires it)
	 * </p>
	 * 
	 */
	protected static void performRemoveRole() {
		...
		if (ViewAddRemoveRoles.theRemoveRole.compareTo("<Select a role>") != 0) {
			...
			if (isAdminRemovingOwnAdminRole(ViewAddRemoveRoles.theSelectedUser, 
	 ...
	 * 
	 * <p> Description: This private helper method checks if the admin being modified is trying to
	 * remove their own admin role. </p>
	 * 
	 ...
	 * 
	 * <p> Description: This private helper method checks if there is at least one other admin
	 * in the system besides the one being modified. </p>
	 * 
	 ...
	 * 
	 * <p> Description: This private helper method checks if removing a role would leave the
	 * user with no roles at all. Based on the system requirement, users should have at least
	 * one role. </p>
	 * 
	...
	private static boolean wouldRemoveAllRoles() {
		...
		if (theDatabase.getCurrentAdminRole()) currentRoleCount++;
		if (theDatabase.getCurrentNewRole1()) currentRoleCount++;
		if (theDatabase.getCurrentNewRole2()) currentRoleCount++;
...
//NOT USED - SAVING IN CASE IT'S NEEDED LATER
//	/**********
//	 * <p> Method: showAccessDeniedAlert() </p>
//	 * 
//	 * <p> Description: Displays an alert when a non-admin user tries to access the Add/Remove
//	 * Roles page. </p>
//	 */
...
//		alert.setContentText("Only users with Admin role can manage user roles.");
//		alert.showAndWait();
//	}
	...
	private static void showAdminSelfRemovalAlert() {
		...
		alert.setContentText("You cannot remove your own Admin role. "
				+ "Another admin must remove it for you.");
		alert.showAndWait();
	}
	...
	private static void showLastAdminAlert() {
		...
		alert.setContentText("The system must have at least one Admin user. "
				+ "You cannot remove the last Admin role from the system.");
		alert.showAndWait();
	}
	...
	private static void showRemoveAllRolesAlert() {
		...
		alert.setContentText("Each user must have at least one role. "
				+ "You cannot remove this user's last role.");
		alert.showAndWait();
	}
