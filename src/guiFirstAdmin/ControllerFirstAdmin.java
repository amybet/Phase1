...
public class ControllerFirstAdmin {
	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		...
		String errorCode = InputRecognizer.checkPassword(adminPassword1);
		if(!errorCode.isEmpty()) {
			//ViewNewAccount.text_Password1.setText("");
			//ViewNewAccount.text_Password2.setText("");
			ViewFirstAdmin.label_InvalidPassword.setText(errorCode);
			
			return;
		}
		//Make sure username is valid
		errorCode = InputRecognizer.checkUsername(adminUsername);
		if(!errorCode.isEmpty()) {
			ViewFirstAdmin.label_InvalidUsername.setText(errorCode);	
			return;
		}
    	...
    	guiUserUpdate.ViewUserUpdateFIRST.displayUserUpdate(ViewFirstAdmin.theStage, user);
