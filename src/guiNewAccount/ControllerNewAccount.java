...
public class ControllerNewAccount {
	...
	*/
	 ...
	 */	
		...
		String errorCode = InputRecognizer.checkPassword(password);
		if(!errorCode.isEmpty()) {
			ViewNewAccount.text_Password1.setText("");
			ViewNewAccount.text_Password2.setText("");
			ViewNewAccount.alertInvalidPasswordError.setHeaderText(errorCode);
			ViewNewAccount.alertInvalidPasswordError.showAndWait();
			
			return;
		}
		
		errorCode = InputRecognizer.checkUsername(username);
		if(!errorCode.isEmpty()) {
			ViewNewAccount.alertInvalidUsernameError.setHeaderText(errorCode);
			ViewNewAccount.alertInvalidUsernameError.showAndWait();
			
			return;
		}
		
		//Invalidates the InviteCode
		boolean validInvite = theDatabase.validateInviteCode(ViewNewAccount.theInvitationCode, ViewNewAccount.emailAddress, ViewNewAccount.theRole);
		if(!validInvite) {
			ViewNewAccount.alertInvitationCodeIsInvalid.setContentText("Invitation code is invalid, expired, or already used.");
				...
				compareTo(ViewNewAccount.text_Password2.getText()) == 0)
		{
            ...
            guiUserUpdate.ViewUserUpdateFIRST.displayUserUpdate(ViewNewAccount.theStage, user);
		}
		else {
